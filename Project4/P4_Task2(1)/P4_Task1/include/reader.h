...
//         // === [修改 2] 队列为空时，等待信号 ===
//         if (waitEvent) {
//           // 等待音频线程鸣枪，最长等 100ms (防止死锁)
//           // 一旦有数据 push 进来，这里会瞬间醒来，延迟 < 1ms
//           waitEvent->wait(100);
//         } else {
//           // 如果没传 event，稍微睡一下，防止 CPU 100% 占用
//           Thread::sleep(1);
//         }
//         continue;
//       }
...
//     auto sync = std::deque<float>(LENGTH_PREAMBLE * 8 * LENGTH_OF_ONE_BIT,
//     0); while (!threadShouldExit()) {
//       protectInput->enter();
//       if (input->empty()) {
//         protectInput->exit();
//
...
//
//   // === [修改 4] 新增成员变量 ===
//   juce::WaitableEvent *waitEvent{nullptr};
// };
//
// #endif // READER_H
//
